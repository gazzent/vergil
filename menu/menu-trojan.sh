...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/NevermoreSSH/gazzent/ip/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/NevermoreSSH/gazzent/ip/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/NevermoreSSH/gazzent/ip/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
echo -e "     ${BICyan}[${BIWhite}06${BICyan}] Check XRAY Trojan Config     "
echo -e ""
